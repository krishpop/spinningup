...
from stable_baselines import HER, SAC, TD3
...
def make_reorient_env():
    ...
    wrappers = [gym.wrappers.ClipAction,
                ...
                {'cls': custom_env.CubeRewardWrapper,
                 'kwargs': dict(pos_coef=.2, ori_coef=1.,
    ...
    env_fn = rrc_utils.make_env_fn('real_robot_challenge_phase_1-v1',
...
def make_her_model(env, exp_dir, method_class=SAC, lr=3e-4):
    info_keys = ['is_success', 'is_success_ori_dist', 'dist', 'final_dist',
                 'final_score', 'final_ori_dist']
    model = HER('MlpPolicy', env, method_class, n_sampled_goal=4,
...
def main():
    ...
    exp_dir = make_exp_dir('TD3', 'push_reorient')
    model = make_her_model(env, exp_dir, TD3)
